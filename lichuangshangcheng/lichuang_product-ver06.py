...
def get_deail(url):
    ...
    try:
        ...
        for tbody in tbodys:
            ...
            # if os.path.exists(newTable1):
            #     # 打开excel
            #     rb = xlrd.open_workbook(newTable1, formatting_info=True)  # formatting_info=True 保留原有字体颜色等样式
        ...
        for i in range(2, page_number+1):
            ...
            try:
                for i in range(30):
                    ...
                    time.sleep(2)
                    # # 如果文件存在，则追加。如果文件不存在，则新建
                    # if os.path.exists(newTable):
                    #     # 打开excel
                    #     rb = xlrd.open_workbook(newTable, formatting_info=True)  # formatting_info=True 保留原有字体颜色等样式
                    #     # 用 xlrd 提供的方法获得现在已有的行数
                    #     rn = rb.sheets()[0].nrows
                    #     # 复制excel
                    #     wb = copy(rb)
