...
def get_html(asin):
    ...
    if rating_review != "None":
        print(rating_review)
        regex = re.compile(r'(?P<ratings>.*?) total ratings, (?P<reviews>.*?) with reviews', re.S)
        result = re.finditer(regex, rating_review)

        f = open('rating_review.csv', mode='a', newline="")
        csvwriter = csv.writer(f)
        for item in result:
            # print(item.group('ratings'))
            # print(item.group('reviews'))
            dic = item.groupdict()
...
if __name__ == '__main__':
    with open('asin.txt', 'r', encoding='utf-8') as f:
        asins = f.readlines()
    # print(len(asins))
    for asin in asins[15:100]:
        # print(asin.strip())
        get_html(asin.strip())
        sleep_time = random.choice([1, 2, 3, 4, 5])
