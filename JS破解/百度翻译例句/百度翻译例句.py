...
def get_sign(query):
    with open('p.js', 'r', encoding='utf-8') as f:
        baidu_js = f.read()
    sign = execjs.compile(baidu_js).call('tl', query)
    return sign
...
def jiexi_shuangyu(result):
    # print(result['double'])

    double_liju = json.loads(result['liju_result']['single'])
    for i in double_liju:
        print(i)


def jiexi_collins(result):
    collins_1 = result['dict_result']['collins']['entry']
    for i in collins_1:
        for j in i['value']:
            for k in j['mean_type']:
                for m in k['example']:
                    print(m['ex'])
                    print(m['tran'])


def jiexi_oxford(result):
    pass


def main():
    query = input('请输入要翻译的英语：')
    ...
    # print('例句为：', result)
    # jiexi_shuangyu(result)
    jiexi_collins(result)
    # jiexi_oxford(result)
